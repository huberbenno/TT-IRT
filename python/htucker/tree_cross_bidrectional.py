  ...
  def run(self, eval_f: Callable[[np.array]], n_iter: int=1, eps: float=0., kickrank: int=2, rf=2, force_init: bool=True, verbose: bool=False) -> None:
    ...
    def worker(node, indexset_p, indexset_dims_p):
      if node.isleaf:
        ...
        u,s,v = np.linalg.svd(eval,full_matrices=False)
        r = np.diag(s) @ v
        # ind, C = rect_maxvol(u, tol=1.1, maxK=u.shape[1] + kickrank + rf, min_add_K=kickrank)
        # ind, C = rect_maxvol(u, tol=1.1, maxK=node.dim,min_add_K=max(0, node.dim-u.shape[1]))
        # qmax = u[ind]
        qmax = u
        ind =  np.arange(n)
        self.tensor.cores[node] = np.eye(n)

        self.indexset_list[node] = ind.reshape(-1,1)
        return qmax @ r
      
      else:
        iter_items = [(i, c) for i,c in enumerate(node.children)]
        # iter_items += iter_items[-2::-1]
        # print(iter_items)
        for i, child in iter_items:
