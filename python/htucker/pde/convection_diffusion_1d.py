...
class ConvectionDiffusion1D:
  ...
  def matrix(self, coeff):
    ...
    A2 = self._matrix2(coeff[2], self.N)
  ...
  def _matrix1(coeff, N):
    ...
    for i in range(I):
      ...
      Ai = np.zeros((N+1)**2)
      Ai[1::N+2] = v[i]# set first upper diag
      Ai[N+1::N+2] = -v[i] # set first lower diag
      Ai = Ai.reshape((N+1, N+1))
      Ai[0] = 0 # for dirichlet boundary
      Ai[-1] = 0 # for dirichlet boundary
      A1.append(Ai)
...
class ConvectionDiffusion1D_torch(ConvectionDiffusion1D):
  ...
  def matrix(self, coeff):
    ...
    A2 = [torch.from_numpy(a) for a in self._matrix2(coeff[2].numpy(force=True), self.N)]
  ...
  def solve(self, coeff_A, coeff_b):
    ...
    for i in range(I):
      A = self.matrix(tuple(coeff_A[k][:,i, np.newaxis] for k in range(len(coeff_A))))
      b = self.rhs(tuple(coeff_b[k][:,i, np.newaxis] for k in range(len(coeff_b))))
      U.append(torch.linalg.solve(A[0][0] + A[1][0] + A[2][0], b[0][0]).reshape((-1,1)))
