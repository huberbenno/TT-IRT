...
\subsubsection{Exact low rank}

Test with affine parameter
\begin{equation*}
  \Bv(j_0, j_1, \dots, j_d)
  = \sum_{k=1}^d k^{-2} j_k \psi_k(j_0)
  , \qquad \psi_k \colon \R \rightarrow \R
\end{equation*}
for $d=20$, $n_{\{k\}} = 5$ for $k=1, \dots, d$, $n_{0} = 100$.

\begin{itemize}
  \item See \cref{fig:cross_affine}
  \item efficient recovery of exact representation up to rounding errors: \num{30000} evaluations for a tensor with \num{9.5e15} entries
  \item balanced binary tree better than linear tree
  \item outperforms ttpy \texttt{rect\_cross} in terms of samples, which uses \num{530000} samples for exact recovery, even with linear tree
\end{itemize}

\begin{figure}
\centering
\begin{subfigure}{.49\textwidth}
  \includegraphics[width=\textwidth]{./figs/cross/cross_affine_linear_tree.png}
  \caption{linear tree}
\end{subfigure}
\begin{subfigure}{.49\textwidth}
  \includegraphics[width=\textwidth]{./figs/cross/cross_affine_balanced_binary_tree.png}
  \caption{balanced binary tree}
\end{subfigure}
\caption{Cross testing with tensor of exact low rank from affine parameter}
\label{fig:cross_affine}
\end{figure}

\subsubsection{Approximate low rank}

Test with full rank parameter with algebraic decay
\begin{equation*}
  \Bv(j_0, j_1, \dots, j_d)
  = \exp \left( \sum_{k=1}^d k^{-2} j_k \psi_k(j_0) \right)
  , \qquad \psi_k \colon \R \rightarrow \R
\end{equation*}
for $d=20$, $n_{\{k\}} = 5$ for $k=1, \dots, d$, $n_{0} = 100$.

\begin{itemize}
  \item See \cref{fig:cross_exp}
  \item proper convergence
  \item no pronounced advantage of balanced binary tree compared to linear tree
  \item still outperforms ttpy \texttt{rect\_cross} in terms of samples, which uses $\sim\num{1e6} $ samples for the same error
\end{itemize}

\begin{figure}
\centering
\begin{subfigure}{.49\textwidth}
  \includegraphics[width=\textwidth]{./figs/cross/cross_exp_linear_tree2.png}
  \caption{linear tree}
\end{subfigure}
\begin{subfigure}{.49\textwidth}
  \includegraphics[width=\textwidth]{./figs/cross/cross_exp_balanced_binary_tree.png}
  \caption{balanced binary tree}
\end{subfigure}
\caption{Cross testing with tensor of full rank with decay, target error \num{1e-4}}
\label{fig:cross_exp}
\end{figure}
...
In order to facilitate a more generic formulation of the algorithm, we consider a modified tree structure, where the special (physical) dimension is the 'parent' of the root node, see \cref{fig:alsc_modified_tree}.
