...
In order to sparsify the reduced systems, we also define projection $\tilde{A}^{(>\alpha)}_\beta$. 
...
\begin{equation}
  ...
  \prod_{\gamma \in N(\alpha) \setminus {\beta}} \tilde{A}^{(>\alpha)}_\gamma[j_{k(\beta)}^{\overline{\Bu}} j_{k(\beta)}^{\Bu} j_{k(\beta)}^{\BA}]
\end{equation}
...
Due to the choice of $\BE$ and $\Bu^{(\alpha)}$, $\tilde{A}^{(>\alpha)}_\beta$ for $\beta \in S(\alpha)$ is given by 
\begin{equation}
  \label{eq:right_proj_reduced)}
  \tilde{A}^{(>\alpha)}_\beta
  = \overline{\Bu^{(\alpha)}}[(j_k^{\overline{\Bu}})_{k \in K}] \
  ...
  \prod_{\gamma \in N(\alpha) \setminus {\beta}} \tilde{A}^{(>\alpha)}_\gamma[j_{k(\beta)}^{\overline{\Bu}} j_{k(\beta)}^{\Bu} j_{k(\beta)}^{\BA}]
\end{equation}
which is the partial evaluation of $\BA$ at the (nested) maxvol indices.

\paragraph{Leaf nodes}

By definition, a leaf node $\alpha$ has no children and $N(\alpha)$ only contains the parent node $\beta$.
Thus the left projection is given by
\begin{equation*}
  A^{(<\alpha)}
  = \left( \Bu^{(\alpha)} \right)^H \BA^{(\alpha)}   \Bu^{(\alpha)}
\end{equation*} 
and the right projection is given by the same expression.
The corresponding Petrov-Galerkin equation is is given by sampling the core at the maxvol indices $\CI$ of $ \Bu^{(\alpha)}$, i.e.
\begin{equation*}
  \tilde{A}^{(> \alpha)}
  = \Bu^{(\alpha)}[\CI, :]
\end{equation*} 
